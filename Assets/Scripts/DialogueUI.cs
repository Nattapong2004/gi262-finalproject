...
public class DialogueUI : MonoBehaviour
{
    ...
    public void Setup(DialogueSequen sequen)
    {
        ...
        // ensure the sequen knows which DialogueUI is controlling it (so SelectChoice can call ShowDialogue)
        if (InteractNpcSequen != null)
        {
            InteractNpcSequen.dialogueUI = this;
        }
        // find UIManager and hide the stamina UI while in dialog
        uiManager = UnityEngine.Object.FindAnyObjectByType<UIManager>();
        if (uiManager != null)
        {
            uiManager.HideStaminaUI();
        }
    ...
    public void HideDialogue()
    {
        ...
        ClearChoices();
        // restore stamina UI when dialogue closed
        if (uiManager == null)
        {
            uiManager = UnityEngine.Object.FindAnyObjectByType<UIManager>();
        }
